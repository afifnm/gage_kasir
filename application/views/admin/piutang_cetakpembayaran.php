<?php
	...
	$pdf->AddPage();
	$pdf->SetFont('times','B',12);
	$pdf->Cell(0 ,5,'PEMBAYARAN PIUTANG '.mediumdate_indo($tanggal1).' SAMPAI '.mediumdate_indo($tanggal2).' '.$aa,0,1,'C');
	$pdf->Cell(130 ,12,'',0,0);
	...
	$pdf->Cell(30 ,5,'TGL.  BAYAR',1,0,'C');
	$pdf->Cell(30 ,5,'DIBAYAR',1,0,'C');
    ...
    foreach ($data3 as $uu) {
    	if($uu['nominal']>0){
			...
			$pdf->Cell(30 ,4,'Rp. '.number_format($uu['nominal']),1,0,'R');
			$pdf->Cell(30 ,4,'Rp. '.number_format($uu['sisa_tagihan']),1,0,'R');
			$pdf->Cell(20 ,4,$uu['pembayaran'],1,0,'C');
			$sum1 = $sum1+$uu['nominal'];
